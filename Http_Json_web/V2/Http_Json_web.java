...
public class Http_Json_web {
    ...
    static class AuthHandler implements HttpHandler {
        ...
        /*private void handleVideo(HttpExchange exchange, String file, String type) throws IOException {
            File video = new File(file);

            exchange.getResponseHeaders().add("Content-Type", "video/" + type);

            if (video.exists()) {
                sendResponseVideo(exchange, video);
            }else {
                sendResponse(exchange, 404, "Vídeo não encontrado!");
            }
        }*/

        private void handleAudio(HttpExchange exchange, String file, String type) throws IOException {
            File audio = new File(file);

            exchange.getResponseHeaders().add("Content-Type", "audio/" + type);

            if (audio.exists()) {
                sendResponseVideo(exchange, audio);
            }else {
                sendResponse(exchange, 404, "Áudio não encontrado!");
            }
        }

        private void handleImage(HttpExchange exchange, String file, String type) throws IOException {
            File image = new File(file);

            exchange.getResponseHeaders().add("Content-Type", "image/" + type);

            if (image.exists()) {
                sendResponseVideo(exchange, image);
            }else {
                sendResponse(exchange, 404, "Vídeo não encontrado!");
            }
        }

        private Boolean webAuth(HttpExchange exchange, String nome) throws IOException {
            ...
            if (auth == null || !auth.startsWith("Basic ")) {
                exchange.getResponseHeaders()
                        .add("WWW-Authenticate", "Basic realm=\"" + nome + "\"");
                exchange.sendResponseHeaders(401, -1);
                return false;
            }

            codemaxymiller.veralllinha("config/html/users-auth.txt");
            boolean loginok = false;
            for (int loop = 1; loop < codemaxymiller.veralllinhanumeroout; loop++) {
                if (!loginok) {
                    String  user = extrairJson(codemaxymiller.veralllinhaout[loop], "user");
                    String  pass = extrairJson(codemaxymiller.veralllinhaout[loop], "pass");
                    //sendResponse(exchange, 200, "pass: " + pass + " user: " + user);
                    //LOGGER.info("loop: " + loop + " pass: " + pass + " user: " + user);
                    String  esperado = "Basic " + Base64.getEncoder()
                            .encodeToString((user + ":" + pass).getBytes());

                    //LOGGER.info("Testando user=" + user);

                    if (auth.equals(esperado)) {
                        loginok = true;
                        LOGGER.info("LOGIN OK | IP: " + ip + " | USER: " + user);
                        break;
                    }
                }
            }
            ...
            if (!loginok) {
                exchange.getResponseHeaders()
                        .add("WWW-Authenticate", "Basic realm=\"" + nome + "\"");
                exchange.sendResponseHeaders(401, -1);
                LOGGER.warning("LOGIN FAIL | IP: " + ip + " | AUTH: " + auth);
                return false;
            }
            return true;
        ...
        private void sendResponseVideo(HttpExchange exchange, File responsebody) throws IOException {


            exchange.sendResponseHeaders(200, responsebody.length());
            try (OutputStream os = exchange.getResponseBody()) {
                Files.copy(responsebody.toPath(), os);
            }
        }
